...
ggplot(polls.data, aes(x=Date, y=Obama.Romney, colour = isNational, alpha=0.6)) +
...
<<support_spend, echo=FALSE, warning=FALSE, fig.width=4, fig.height=4.5, out.width='.45\\textwidth', fig.pos='H', fig.cap='Change in polling over change in spending by candidate', fig.show='hold'>>=

## One Week Lag
ggplot(final.df3[-1,], aes(x=ObamaSpendChange, y=ObamaPollChange, colour = week)) +
...
ggplot(final.df3[-1,], aes(x=RomneySpendChange, y=RomneyPollChange, colour = week)) +
